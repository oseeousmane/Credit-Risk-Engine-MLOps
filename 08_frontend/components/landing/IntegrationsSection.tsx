      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/[0.04] border border-white/[0.08] mb-6 shadow-sm">
            <span className="text-[11px] font-bold uppercase tracking-[0.25em] text-[#3ECF8E]">{header.badge}</span>
          </div>
        ...
        <div className="relative p-1 rounded-3xl bg-[#050505] border border-white/[0.03]">
          {/* Decorative network lines */}
          <div className="absolute inset-0 opacity-20 pointer-events-none" 
               style={{ 
                 backgroundImage: 'radial-gradient(circle at center, #3ECF8E 1px, transparent 1px)', 
                 backgroundSize: '32px 32px' 
               }} 
          />
          <div className="absolute top-1/2 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#3ECF8E]/20 to-transparent transform -translate-y-1/2" />
          ...
          <div className="relative z-10 grid md:grid-cols-4 gap-px bg-white/[0.02]">
            {integrations.map((integration, i) => (
              <motion.div 
                ...
                className="group p-8 bg-[#050505] hover:bg-[#080808] transition-colors duration-500 flex flex-col items-center text-center"
              >
                <div className="w-12 h-12 rounded-xl bg-white/[0.02] border border-white/[0.04] flex items-center justify-center mb-5 group-hover:bg-[#3ECF8E]/10 group-hover:border-[#3ECF8E]/20 transition-all duration-500 shadow-sm relative">
                  <div className="absolute inset-0 bg-[#3ECF8E]/20 blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                  <integration.icon className="w-5 h-5 text-zinc-500 group-hover:text-[#3ECF8E] transition-colors relative z-10" />
                </div>
